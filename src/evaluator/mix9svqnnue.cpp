...
{
    using typename WeightLoader<mix9svq::Weight>::WeightPtr;

    std::expected<WeightPtr, std::string> load(std::istream &in, EmptyLoadArgs args)
    {
        auto w = make_large_page<Weight>();
    ...
    {
        simd::preprocessLinear<OutSize * 2, InSize>(b.up1.weight);
        simd::preprocessLinear<OutSize * 2, InSize>(b.up2.weight);
        simd::preprocessLinear<OutSize, OutSize>(b.down.weight);
    }
};

static WeightRegistry<StandardHeaderLoader<Mix9svqWeightLoader>> WeightReg;
...
starBlock(int8_t output[OutSize], int8_t input[InSize], const StarBlockWeight<OutSize, InSize> &w)
{
    ...
    simd::linear<OutSize * 2, InSize>(upi32, input, w.up1.weight, w.up1.bias);
    simd::crelu<OutSize * 2, 128>(up1, upi32);

    simd::linear<OutSize * 2, InSize>(upi32, input, w.up2.weight, w.up2.bias);
    ...
    simd::linear<OutSize, OutSize, true>(outputi32, dotsum, w.down.weight, w.down.bias);
...
{
    assert(pieceColor == C_BLACK || pieceColor == C_WHITE);
...
void Accumulator::evaluatePolicy(const Weight &w, PolicyBuffer &policyBuffer)
{
    ...
    for (int y = 0, outerIdx = outerBoardSize + 1; y < boardSize; y++, outerIdx += 2) {
        for (int x = 0; x < boardSize; x++, outerIdx++) {
            Move move {x, y};
            if (!policyBuffer.getComputeFlag(move))
                continue;
            ...
            policyBuffer(move) = policy;
        }
    }
}

std::expected<Evaluator, std::string>
Evaluator::create(int boardSize, int numaNodeId, std::filesystem::path weightPath)
{
    CompressedWrapper<StandardHeaderLoader<Mix9svqWeightLoader>> loader(
        Compressor::Type::LZ4_DEFAULT);

    if (boardSize < 1 || boardSize > 32)
        return std::unexpected("mix9svq nnue: board size " + std::to_string(boardSize)
                               + " is not supported, must be in [1, 32].");

    Time startTime     = Now();
    bool printLoadInfo = false;
    loader.setHeaderValidator([&](StandardHeader header, auto &args) -> std::optional<std::string> {
        ...
        if (std::find(header.supportedBoardSizes.begin(),
    ...
    std::unique_ptr<Weight, WeightDeleter> weights[SIDE_NB];
    for (const auto &[side, weightPath] : {
             std::make_pair(P_FIRST, weightPath),
             std::make_pair(P_SECOND, weightPath),
         }) {
        auto result = WeightReg.loadWeightFromFile(loader,
                                                   weightPath,
                                                   numaNodeId,
                                                   {{}, boardSize, 0, weightPath});

        if (result)
            weights[side] = std::unique_ptr<Weight, WeightDeleter> {result.value()};
        else
...
void Evaluator::WeightDeleter::operator()(Weight *w) const
{
    if (w)
        WeightReg.unloadWeight(w);
}
...
Evaluator::Evaluator(int                                    boardSize,
    ...
    : weight {std::move(blackWeight), std::move(whiteWeight)}
{
    accumulator[P_FIRST]  = std::make_unique<Accumulator>(boardSize);
    accumulator[P_SECOND] = std::make_unique<Accumulator>(boardSize);

    int numCells = boardSize * boardSize;
    moveCache[P_FIRST].reserve(numCells);
    moveCache[P_SECOND].reserve(numCells);
}

void Evaluator::reset()
{
    for (Player player : {P_FIRST, P_SECOND}) {
        moveCache[player].clear();
        accumulator[player]->clear(*weight[player]);
    }
}

void Evaluator::move(Color color, int x, int y)
{
    addCache(color, x, y, false);
}

void Evaluator::undo(Color color, int x, int y)
{
    addCache(color, x, y, true);
}

Value Evaluator::evaluateValue(Player side)
{
    // Apply all incremental update for both sides and calculate value
    clearCache(static_cast<Color>(side));
    auto [win, loss, draw] = accumulator[side]->evaluateValue(*weight[side]);

    return Value(win, loss, draw, true);
}

void Evaluator::evaluatePolicy(Player side, PolicyBuffer &policyBuffer)
{
...
void Evaluator::clearCache(Color side)
{
    for (MoveCache &mc : moveCache[side]) {
        if (side == C_WHITE) {
            mc.oldColor = ~mc.oldColor;
            mc.newColor = ~mc.newColor;
        }

        if (mc.oldColor == C_EMPTY)
